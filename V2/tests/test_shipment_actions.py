    ...
    with get_connection() as connection:
        if name == "GetShopStatus":
            ...
            row = connection.execute("""SELECT r.shipment_id::text, t.status AS task_status, t.error_code, s.carrier, s.tracking_number, s.version
                ...
                WHERE r.company_id = %s AND r.shop_id = %s AND r.external_order_id = %s""", (user.company_id, shop_id, order_id)).fetchone()
        else:
...
@pytest.fixture()
def shipment_action_runtime(seeded_database: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> tuple[UserContext, str]:
