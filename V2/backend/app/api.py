...
def current_auth(authorization: str = Header(default="")) -> AuthContext:
...
AuthDependency = Annotated[AuthContext, Depends(current_auth)]


def conversation_snapshot(auth: AuthContext, conversation_id: str) -> dict[str, object]:
    ...
    return {
        ...
        "case": show_case(case_row["case_id"], auth) if case_row else None,
        "actions": [show_action(auth, row["action_id"]) for row in action_rows],
        "ticket": show_ticket(auth, ticket_row["ticket_id"]) if ticket_row else None,
    }
...
def start_conversation(auth: AuthDependency) -> dict[str, object]:
    if auth.role == "engineer":
        raise PermissionError("Engineer identities cannot create merchant conversations")
    return conversation_snapshot(auth, create_conversation(auth.company_id, auth.user_id))


@app.get("/api/v1/conversations/{conversation_id}")
def get_conversation_snapshot(conversation_id: str, auth: AuthDependency) -> dict[str, object]:
    return conversation_snapshot(auth, conversation_id)


@app.post("/api/v1/conversations/{conversation_id}/messages")
def send_message(conversation_id: str, request: MessageRequest, auth: AuthDependency) -> dict[str, object]:
    turn = process_conversation_message(auth, request.question, conversation_id, request.retrieval_mode)
    return {"turn": turn, "snapshot": conversation_snapshot(auth, conversation_id)}


@app.post("/api/v1/cases/{case_id}/actions")
def propose_action(case_id: str, request: ProposalRequest, auth: AuthDependency) -> dict[str, object]:
    if request.action_type == "recover_shipment":
        return propose_shipment_recovery(auth, case_id, request.enable_shipment_sync)
    return propose_order_recovery(auth, case_id, request.enable_order_sync)


@app.get("/api/v1/actions/{action_id}")
def get_action(action_id: str, auth: AuthDependency) -> dict[str, object]:
    return show_action(auth, action_id)


@app.post("/api/v1/actions/{action_id}/decision")
def action_decision(action_id: str, request: DecisionRequest, auth: AuthDependency) -> dict[str, object]:
    return decide_action(auth, action_id, request.decision)


@app.post("/api/v1/actions/{action_id}/execute")
def action_execution(action_id: str, auth: AuthDependency) -> dict[str, object]:
    return execute_action(auth, action_id)


@app.post("/api/v1/tickets")
def request_engineer(request: TicketRequest, auth: AuthDependency) -> dict[str, object]:
    return create_ticket(auth, request.conversation_id, "user_requested", request.reason)


@app.get("/api/v1/tickets/{ticket_id}")
def get_ticket(ticket_id: str, auth: AuthDependency) -> dict[str, object]:
    return show_ticket(auth, ticket_id)


@app.post("/api/v1/tickets/{ticket_id}/recheck")
def ticket_recheck(ticket_id: str, auth: AuthDependency) -> dict[str, object]:
    return recheck_ticket(auth, ticket_id)


@app.get("/api/v1/tickets/{ticket_id}/export", response_class=HTMLResponse)
def ticket_export(ticket_id: str, auth: AuthDependency) -> str:
    return render_ticket_html(show_ticket(auth, ticket_id))


@app.get("/api/v1/engineer/tickets")
def engineer_tickets(auth: AuthDependency) -> list[dict[str, object]]:
    return list_engineer_tickets(auth)
