...
from app.customer_document_ingestion import create_chunk_ids, load_customer_document
...
def test_import_customer_documents_uses_langchain_splitter_and_database(monkeypatch: pytest.MonkeyPatch) -> None:
    customer_document = Document(page_content="ResolveAI can send an order notification when an order changes.", metadata={"title": "Overview", "visibility": "CUSTOMER", "feature": "order notifications", "version": "2026.8", "source_uri": "docs/customer/overview.md", "effective_from": date(2026, 8, 1), "effective_to": None})
    ...
    monkeypatch.setattr(customer_document_ingestion, "load_all_customer_documents", lambda: [customer_document])
    monkeypatch.setattr(customer_document_ingestion, "reset_customer_knowledge_table", fake_reset_customer_document_table)
    monkeypatch.setattr(customer_document_ingestion, "get_customer_knowledge_database_client", lambda: customer_document_database)

    imported_chunk_count = customer_document_ingestion.import_customer_documents()
